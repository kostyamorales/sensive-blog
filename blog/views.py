...
def index(request):
    ...
    most_fresh_posts = Post.objects.annotate(comments_count=Count('comments')).order_by(
        '-published_at').prefetch_related('author').fetch_with_tags()[:5]
...
def tag_filter(request, tag_title):
    ...
    related_posts = tag.posts.annotate(comments_count=Count('comments')).prefetch_related('author').fetch_with_tags()[
                    :20]
